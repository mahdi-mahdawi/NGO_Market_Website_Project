...
class Template {
     ...
     * @return	void
     */
    public function set_js($array = array()) {
        $temp = '';
        if (!empty($array)) {
            foreach ($array as $item) {
                $temp .= '<script type="text/javascript" src="' . base_url() . $item . '"></script>' . "\n";
            }
        }
     ...
     * @return	void
     */
    public function set_css($array = array()) {
        $temp = '';
        if (!empty($array)) {
            foreach ($array as $item) {
                $temp .= '<link rel="stylesheet" href="' . base_url() . $item . '" type="text/css" />' . "\n";
            }
        }
     ...
     * @return	void
     */
    public function load_asset($lib = array()) {
        $asset = config_item('asset_lib');
        ...
        foreach ($lib as $row) {
            if (array_key_exists($row, $asset)) {
                if (isset($asset[$row]['css'])) {
                    $this->set_css($asset[$row]['css']);
                }

                if (isset($asset[$row]['js'])) {
                    $this->set_js($asset[$row]['js']);
                }
            }
        }
    }
    ...
    public function build($view, $data = array(), $return = FALSE) {
        ...
        $template['js'] = $this->js_array;
        $template['css'] = $this->css_array;
        $template['subheader'] = $this->_subheader;
